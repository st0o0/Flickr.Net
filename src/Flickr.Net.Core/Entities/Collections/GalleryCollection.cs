...
// /// <summary> /// The ID photo that these galleries contain if called from <see ///
// cref="IFlickrGalleries.GetListForPhotoAsync(string, int, int, CancellationToken)"/>. ///
// </summary> public string PhotoId { get; set; }

// void IFlickrParsable.Load(System.Xml.XmlReader reader) { if (reader.LocalName != "galleries") {
...
// case "user_id": UserId = reader.Value; break;

// case "photo_id": PhotoId = reader.Value; break;

// default: UtilityMethods.CheckParsingException(reader); break; } }

// reader.Read();

// while (reader.LocalName == "gallery") { Gallery r = new(); ((IFlickrParsable)r).Load(reader);
// Add(r); }
