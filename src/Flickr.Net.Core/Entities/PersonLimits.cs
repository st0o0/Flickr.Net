...
// if (!reader.ReadToFollowing("photos")) { throw new ResponseXmlException("Unable to find
// \"photos\" element in response."); }

// while (reader.MoveToNextAttribute()) { switch (reader.LocalName) { case "maxdisplaypx":
// MaximumDisplayPixels = reader.ReadContentAsInt(); break;

// case "maxupload": MaximumPhotoUpload = reader.ReadContentAsLong(); break;

// default: UtilityMethods.CheckParsingException(reader); break; } }

// if (!reader.ReadToFollowing("videos")) { throw new ResponseXmlException("Unable to find
// \"videos\" element in response."); }

// while (reader.MoveToNextAttribute()) { switch (reader.LocalName) { case "maxduration":
// MaximumVideoDuration = reader.ReadContentAsInt(); break;

// case "maxupload": MaximumVideoUpload = reader.ReadContentAsLong(); break;
